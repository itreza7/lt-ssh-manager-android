...
public class TextSelectionCursorController implements CursorController {
    ...
    public void setActionModeCallBacks() {
        final ActionMode.Callback callback = new ActionMode.Callback() {
            ...
            public boolean onCreateActionMode(ActionMode mode, Menu menu) {
                ...
                // Offer "Open link" when the (auto-word-expanded) selection is a web URL.
                if (asUrl(getSelectedText()) != null) {
                    menu.add(Menu.NONE, ACTION_OPEN_URL, Menu.NONE, "Open link").setShowAsAction(show);
                }
    ...
    /** Returns a launchable web URL if the (single-token) selection looks like a link, else null. */
    private String asUrl(String text) {
        if (text == null) return null;
        String t = text.trim();
        if (t.isEmpty() || t.contains(" ")) return null;
        if (URLUtil.isValidUrl(t) &&
            (t.startsWith("http://") || t.startsWith("https://") || t.startsWith("ftp://"))) {
            return t;
        }
        if (t.startsWith("www.") && t.length() > 4) {
            return "https://" + t;
        }
        return null;
    }

    /** Open the selected URL in the user's browser (or whichever app handles the scheme). */
    private void openUrl(String text) {
        String url = asUrl(text);
        if (url == null) return;
        try {
            ...
            // No handler app / malformed URI — ignore.
        }
    }
